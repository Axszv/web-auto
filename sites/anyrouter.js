// sites/anyrouter.js — GitHub OAuth 登录 (反检测优化版)
    ...
    if (!isLoggedIn) {
      ...
      for (let attempt = 1; attempt <= 3 && !oauthSuccess; attempt++) {
        ...
        try {
          ...
          console.log('anyrouter: after login URL:', afterLoginUrl);

          if (afterLoginUrl.includes('u2f')) {
            console.log('anyrouter: U2F after login, aborting...');
            await saveGitHubCookies(ctx);
            return { success: true, checkinSuccess: false, needsU2F: true };
          }
          ...
          if (afterLoginUrl.includes('/login/oauth/authorize')) {
            ...
            for (let i = 0; i < 20; i++) {
              ...
              if (current.includes('u2f')) {
                console.log('anyrouter: U2F during callback, aborting...');
                await saveGitHubCookies(ctx);
                break;
              }
            }
      ...
      if (!oauthSuccess) {
        console.log('anyrouter: OAuth failed');
      }
    }
